...
class MatchSimulator:
    def __init__(self, team_a_model, team_b_model, logging_enabled=True):
...
def get_prematch_odds(user_team_id, enemy_team_id, simulations=100):
    ...
    enemy_team_home, enemy_team_away = MatchTeam(enemy_team_model, is_home=True), MatchTeam(enemy_team_model, is_home=False)

    def _run_fixture_sims(home_team_model, away_team_model):
        wins, draws, losses, goals_for, goals_against = 0, 0, 0, 0, 0
        for _ in range(simulations):
            simulator = MatchSimulator(home_team_model, away_team_model, logging_enabled=False)
    ...
    home_fixture_probs = _run_fixture_sims(user_team_model, enemy_team_model)
    away_fixture_probs = _run_fixture_sims(enemy_team_model, user_team_model)
